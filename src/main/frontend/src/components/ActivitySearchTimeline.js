...
export default function ActivitySearchTimeline(props) {
  // getActivityListTEST
  // 검색결과(참조데이터:활동내역 리스트)
  const [searchActivityList, setSearchActivityList] = useState([]);
...
var groupBy = function(xs, key) {
  ...
  useEffect(() => {
      const fetchSearchActivityList = async () => {
        try {
          if(!props.searchGoalIdList.length){
            setSearchActivityList([])
          } else{
            ...
            // 리스트에 세팅하기(원본)
            setSearchActivityList(result.data.activityList);
  ...
  if (searchActivityList.length === 0) {
    ...
    return (
      ...
      <Timeline position="alternate">
        {searchActivityList && Object.entries(tmpList).map((activityPerDay, index) => (
